...
from recognize.get_models import download_models

from recognize.ocr.handwriting_line_recognition import Network as HandwritingRecognitionNet, handwriting_recognition_transform
from recognize.ocr.handwriting_line_recognition import decode as decoder_handwriting, alphabet_encoding
from recognize.ocr.paragraph_segmentation_dcnn import SegmentationNetwork, paragraph_segmentation_transform
from recognize.ocr.utils.beam_search import ctcBeamSearch
from recognize.ocr.utils.denoiser_utils import SequenceGenerator
from recognize.ocr.utils.encoder_decoder import Denoiser, ALPHABET, encode_char, EOS, BOS
from recognize.ocr.utils.expand_bounding_box import expand_bounding_box
from recognize.ocr.utils.iam_dataset import IAMDataset, crop_handwriting_page
from recognize.ocr.utils.preprocess import histogram, all_togather
from recognize.ocr.utils.sclite_helper import ScliteHelper
from recognize.ocr.utils.word_to_line import sort_bbs_line_by_line, crop_line_images
from recognize.ocr.word_and_line_segmentation import SSD as WordSegmentationNet, predict_bounding_boxes
...
class recognization_test():
    def __init__(self, image_name="elyaz2.jpeg", filter_number=1, form_size=(1120, 800), device=None, num_device=1,
        ...
        # self.image = self.image[..., 0]
        self.htr = recognization(self.image, form_size=form_size, device=self.device, show=self.show)
...
if __name__ == "__main__":
    ...
    # htr_test = recognization_test(show=True, device=device)
    # result = htr_test()

    image = mx.image.imread("elyaz2.jpeg")
    image = image.asnumpy()
    recog = recognization(image, device=device)
    result = recog()
    print(result)
