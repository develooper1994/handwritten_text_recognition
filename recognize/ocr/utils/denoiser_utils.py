...
from recognize.ocr.utils.encoder_decoder import decode_char


class SequenceGenerator:

    def __init__(self, sampler, language_model, vocab, ctx_nlp, tokenizer=nlp.data.SacreMosesTokenizer(),
    ...
    def generate_sequences(self, inputs, begin_states, sentence):
        ...
        sentence_tokenized = [i.replace("&quot;", '"').replace("&apos;", "'").replace("&amp;", "&") for i in
                              self.tokenizer(sentence)]
        sentence_correct = True
        for token in sentence_tokenized:
            if (token not in self.vocab or self.vocab[token] > 400000) and token.lower() not in ["don't", "doesn't",
                                                                                                 "can't", "won't",
                                                                                                 "ain't", "couldn't",
                                                                                                 "i'd", "you'd", "he's",
                                                                                                 "she's", "it's",
        ...
        for i, sample in enumerate(samples):
            ...
            tokens = [i.replace("&quot;", '"').replace("&apos;", "'").replace("&amp;", "&") for i in
        ...
        for tokens in best_tokens__:
            if len(tokens) > 1:
                ...
                output, _ = self.language_model(mx.nd.array(inputs).expand_dims(axis=1).as_in_context(self.ctx_nlp),
                                                hidden)
